...
3.Binary description : BRISK
  ...
  -抽樣方式 : 同心環採樣(在關鍵點周為同心圓擴散平均取樣)
  
  -採樣的成對 : 隨機取兩個抽樣集的pixel，計算其座標的歐基里德距離
    
    -長對(距離較大) : 用以描述關鍵點所在區域的方向性
    
    -短對(距離較小) : 用以描述像素強度的落差
    
  -方向補償 : 長對的方向描述可以用以貢獻計算出其方向不變性
  
  -短隊則可以用來最後生成特徵向量 : 共有512個短對 ->512個0,1->64 unsigned 8bit
